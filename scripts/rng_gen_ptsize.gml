///rng_gen_ptsize(grid, coord pts, min dist, border 0-1, 1:square 0:circle, avg rad, type);

    // Generates disks at all coord pts. and randomly generates smalles disks 
    // along the connecting path to make the structure more interesting
    ...
    for(var i = 0; i < ds_list_size(xl); i++){
        for(var j = i+1; j < ds_list_size(xl); j++){    
            ...
            {
                grid[# ax-1, ay-1] = type;
                ds_grid_set_disk(grid, ax, ay, round(rad * random_range(0.5, 1.5)), type);
                grid[# bx-1, by-1] = type;
                ds_grid_set_disk(grid, bx, by, round(rad * random_range(0.5, 1.5)), type);
                ...
                for(var l = 0; l <= ab; l++){
                    ...
                    if random(1) < 0.25
                    {
                        ds_grid_set_disk(grid, lx-1, ly-1, round(0.3 * rad * random_range(0.5, 1.5)), type);
                    }                    
                }
            }
        }
